...
if ~vrb,
    disp_str(50,'fdlmi_mincx_yalmip')
else
    disp_str(51,'fdlmi_mincx_yalmip')
end

export_date=date;
str{1}=['%% Created by fdlmi_mincx_yalmip on ' export_date];
str{2}='\n%% Load coefficient matrices ...';
str{3}='load fdlmi_mincx_yalmip_exe';
...
for k=1:length(lmi_log),
    ...
    str{sc} = ['F = F + [LMI',ks,'<=-',sft_v,'*eye(size(LMI',ks,...
        ',1))];'];
    eval(str{sc});
    sc = sc+1;
end
...
if ~vrb
    save fdlmi_mincx_yalmip_exe lmiparameter CST*
    
    if ~prob_feas
        save fdlmi_mincx_yalmip_exe COEFF cmatrix -append
    end
    
    if freq_dep_lmi_flag
        save fdlmi_mincx_yalmip_exe state* -append
    end
    
    if exist('struct1','var')
        save fdlmi_mincx_yalmip_exe struct* -append
    end
    
    if exist('l_1_1','var')
        save fdlmi_mincx_yalmip_exe l_* -append
    end
    
    if exist('r_1_1','var')
        save fdlmi_mincx_yalmip_exe r_* -append
    end
    
    fid=fopen('fdlmi_mincx_yalmip_exe.m','wt');
    for i1=1:sc-1,
        fprintf(fid,[str{i1} '\n']);
    end
    fclose(fid);
