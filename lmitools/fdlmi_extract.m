function E=fdlmi_extract(obj)
% function E=fdlmi_extract(obj)
%
% processes log file of "fdlmi" environment
...
if ~strcmp(A.name,'fdlmi'),
   error('This is not an "fdlmi" environment')
end
...
if vrb,
   disp('fdlmi_extract: processing the abst log information ... ')
end
         ...
         case num_op('plus'),
           ...
           if (v1~=v2)|(h1~=h2),
              ...
              elseif (v2==1)&(h2==1),
                 ...
                 H{k}=mvrtct(H{a1},H{a2}*ones(1,h1));
                 X{k}=[X{a1} X{a2}];
                 if v1==h1,
                    msg1='ambiguous scalar in matrix expression found in log # ';
...
for k=1:length(lmi_log)
    ...
    if (v1~=v2)|(h1~=h2),
       ...
       GG=mhrzct(s(1)*G{a1},s(2)*G{a2});  % horizontal concatenation
       HH=mvrtct(H{a1},H{a2});            % vertical concatenation
       X{lmi_log(k)}=[X{a1} X{a2}];         
    end
...
if prob_feas==0,      % whether optimization problem?
   ...
   [AH,BH,CH,DH]=ssdata(HH);
   [AG,BG,CG,DG]=ssdata(GG);
   if ~(isempty(AH) & isempty(AG))
      error('cost function is frequency dependent?!!')
   end
   DD=[DH;DG'];
   C{k}=DD;
end
