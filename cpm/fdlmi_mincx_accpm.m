function [cost,xopt]=fdlmi_mincx_accpm(varargin)
% 
% Analytical Center Cutting Plane Method for solving frequency dependent LMI 
%
% function [cost,xopt]=fdlmi_mincx_accpm(obj,options)
%
...
while ~done & counter<=max_iteration,
 ...
 % ---- 1. check the non-freq depedent LMIs ----
    ...
    for flcnt1=1:length(nfdeplmi)
        ...
        [cholR,cholp]=chol(-MX);
        if cholp>0,           
           %% [V,D]=eigs(MX,1,'LR',eig_options); %% eigs is not very syable!
           [V,D] = eig(MX);
           [D IND] = max(diag(D));
           V = V(:,IND);               
           if ~(D> -positive_ind)
    ...
    if ~go_home   % i.e., the non-freq LMIs passed
        ...
        for flcnt1=1:length(fdeplmi)
            ...
            if cholp>0,
               ...
               %% [V,D]=eigs(R,1,'LR',eig_options);  %% eigs is not very syable!
               [V,D] = eig(R);
               [D IND] = max(diag(D));
               V = V(:,IND);               
               if ~(D> -positive_ind)
 ...
 if ~go_home
    if feap
       ...
       % disp(['gamma: ',num2str(obj_now),' V: ',num2str(Vol),' size of A:',num2str(size(LP_A,1))])
 end 
