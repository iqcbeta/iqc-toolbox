function [gain,estimation]=...
    iqc_estimation_l2gain_DTrep_lmilab(w,v,y,p,q,weight)
% function [gain,estimation]=...
%     iqc_estimation_l2gain_DTrep_lmilab(w,v,y,p,q,weight)
%
%          ---------
...
switch ABST.systemtype
    case 'continuous',
        disp_str(70,'iqc_estimation_l2gain_DTrep_lmilab','continuous')
end
...
if vrb,
    if lmiparameter(5)>100,
        disp_str(50,'iqc_estimation_l2gain_DTrep_lmilab')
    else
        disp_str(51,'iqc_estimation_l2gain_DTrep_lmilab')
    end
end
...
export_date=date;
str{1}=['%% Created by iqc_estimation_l2gain_DTrep_lmilab on ' export_date];
str{2}='\n%% Load coefficient matrices ...';
str{3}='load iqc_estimation_l2gain_DTrep_lmilab_exe';
...
%% main lmi
% [-(bY+bXpsi)+A1'*Xpsi*A1+C'*Pr0*C  A1'*Xpsi*B1+C'*Pr0*D   Ae'   *  ]
% [                 *                B1'*Xpsi*B1+D'*Pr0*D   Be'   *  ]
% [                 *                         *            -bY    *  ] < 0
% [               Theta11^(1/2)*C3    Theta11^(1/2)*D3      0     -I ]
ks=num2str(str2double(ks)+1);
...
%% A1'*Xpsi*B1+[bC3' 0]*Theta0*[bD3;[I 0]]
...
% hatB*hatM
str{sc}=['lmiterm([',ks,' 4 1 hatM],hatB,1);'];
eval(str{sc});
sc=sc+1;
...
% hatB*N*hatC
str{sc}=['lmiterm([',ks,' 4 2 N],hatB,hatC);'];
eval(str{sc});
sc=sc+1;

% hatK
str{sc}=['lmiterm([',ks,' 5 1 hatK],1,1);'];
eval(str{sc});
sc=sc+1;

% X*hatA
str{sc}=['lmiterm([',ks,' 5 2 X],1,hatA);'];
eval(str{sc});
sc=sc+1;

% L*hatC
str{sc}=['lmiterm([',ks,' 5 2 L],1,hatC);'];
eval(str{sc});
sc=sc+1;
...
% hatB*N*[hatF1 hatF2]
str{sc}=['lmiterm([',ks,' 4 3 N],hatB,[hatF1 hatF2]);'];
eval(str{sc});
sc=sc+1;

% X*[hatB1 hatB2]
str{sc}=['lmiterm([',ks,' 5 3 X],1,[hatB1 hatB2]);'];
eval(str{sc});
sc=sc+1;

% L*[hatF1 hatF2]
str{sc}=['lmiterm([',ks,' 5 3 L],1,[hatF1 hatF2]);'];
eval(str{sc});
sc=sc+1;
...
if size(hatA11,1)~=0
    ...
    str{sc}=['lmiterm([',ks,' 5 4 0],-',ouf_str1,...
        ''',',ouf_str1,');'];
    eval(str{sc});
    sc=sc+1;
    
    str{sc}=['lmiterm([',ks,' 5 4 -T12],-',ouf_str2,...
        ''',',ouf_str1,');'];
    eval(str{sc});
    sc=sc+1;
end
...
str{sc}=['lmiterm([',ks,' 5 4 T22],-',ouf_str2,...
    ''',',ouf_str2,');'];
eval(str{sc});
sc=sc+1;

str{sc}=['lmiterm([',ks,' 5 5 X],-1,1);'];
eval(str{sc});
sc=sc+1;

%% B1'*Xpsi*B1+bD'*Pr0*bD
% B1'*Xpsi*B1
if size(A1,1)~=0
...
% [I;0]*Theta22*[I 0]
str{sc}=['lmiterm([',ks,' 3 3 min_gamma],-',ouf_str4,...
    ''',eye(',nb1,')*',ouf_str4,');'];
eval(str{sc});
sc=sc+1;
...
%% Theta11^(1/2)*C3
if size(hatA11,1)~=0
    str{sc}=['lmiterm([',ks,' 6 1 T11],sqrtm(Theta11)*hatC3*',ouf_str1,...
        ''',',ouf_str1,');'];
    eval(str{sc});
    sc=sc+1;
    str{sc}=['lmiterm([',ks,' 6 1 T12],-sqrtm(Theta11)*hatC3*',ouf_str1,...
        ''',',ouf_str2,');'];
    eval(str{sc});
    sc=sc+1;
end
str{sc}=['lmiterm([',ks,' 6 1 0],sqrtm(Theta11)*hatC3*',ouf_str2,...
    '''*',ouf_str2,');'];
eval(str{sc});
sc=sc+1;

str{sc}=['lmiterm([',ks,' 6 1 hatM],sqrtm(Theta11)*hatE2,1);'];
eval(str{sc});
sc=sc+1;

str{sc}=['lmiterm([',ks,' 6 2 0],sqrtm(Theta11)*hatC3);'];
eval(str{sc});
sc=sc+1;

str{sc}=['lmiterm([',ks,' 6 2 N],sqrtm(Theta11)*hatE2,hatC);'];
eval(str{sc});
sc=sc+1;

%% Theta11^(1/2)*D3
str{sc}=['lmiterm([',ks,' 6 3 0],sqrtm(Theta11)*hatD3);'];
eval(str{sc});
sc=sc+1;

str{sc}=['lmiterm([',ks,' 6 3 N],sqrtm(Theta11)*hatE2,[hatF1 hatF2]);'];
eval(str{sc});
sc=sc+1;

%% -I
str{sc}=['lmiterm([',ks,' 6 6 0],-eye(',nc3,'));'];
eval(str{sc});
sc=sc+1;
...
if lmiparameter(5)>100;
    save iqc_estimation_l2gain_DTrep_lmilab_exe A A1 A2 A3 B1 B2 B3...
        Dqp Dqw Bp Bw C1 C2 D1 D2 Cq Cv Cy Dvp Dvw Dyp Dyw...
        lmiparameter A_W B_W C_W D_W
    if exist('state1','var')
        save iqc_estimation_l2gain_DTrep_lmilab_exe state* -append
    end
    if exist('struct1','var')
        save iqc_estimation_l2gain_DTrep_lmilab_exe struct* -append
    end
    if exist('l_1_1','var')
        save iqc_estimation_l2gain_DTrep_lmilab_exe l_* -append
    end
    if exist('r_1_1','var')
        save iqc_estimation_l2gain_DTrep_lmilab_exe r_* -append
    end
    if exist('MpiL_1_1','var')
        save iqc_estimation_l2gain_DTrep_lmilab_exe MpiL_* -append
    end
    if exist('MpiR_1_1','var')
        save iqc_estimation_l2gain_DTrep_lmilab_exe MpiR_* -append
    end
    fid=fopen('iqc_estimation_l2gain_DTrep_lmilab_exe.m','wt');
    for i=1:sc-1
        fprintf(fid,[str{i} '\n']);
    end
    fclose(fid);
