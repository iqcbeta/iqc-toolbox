function [gain,estimation]=...
    iqc_estimation_l2gain_CTeli_yalmip(w,v,y,p,q,weight)
% function [min_gamma,estimation]=...
%     iqc_estimation_l2gain_CTeli_yalmip(w,v,y,p,q,weight)
%
%          ---------
%          |       |
...
switch ABST.systemtype
    case 'continuous',
        psi='[0 1;1 0]';
    case 'discrete',
        disp_str(70,'iqc_estimation_l2gain_CTeli_yalmip','discrete')
end
num_var_pos={};
...
if ~vrb
    disp_str(50,'iqc_estimation_l2gain_CTeli_yalmip')
else
    disp_str(51,'iqc_estimation_l2gain_CTeli_yalmip')
end
...
export_date=date;
str{1}=['%% Created by iqc_estimation_l2gain_CTeli_yalmip on ' export_date];
str{2}='\n%% Load coefficient matrices ...';
str{3}='load iqc_estimation_l2gain_CTeli_yalmip_exe';
...
str{sc}=['M_main_right=[zeros(',nx1,',',nx1,') (X+Xpsie) zeros(',nx1,',',nx3,');',...
    '(X+Xpsie) zeros(',nx2,',',cal_str(nx2,nx3),');',...
...
str{sc}=['M_Gamma_left11=[zeros(',nx1,',',nx1,') eye(',nx1,...
    ') zeros(',nx1,',',nx3,');',...
    'eye(',nx1,') zeros(',nx1,',',cal_str(nx2,nx3),');',...
...
str{sc}=['ouf_Gamma_left11=[eye(',nx1,') zeros(',nx1,',',...
    cal_str(nx2,nx3),');',...
    'T1AT2+Xpsie*hatA T1''*hatB1+Xpsie*hatB1 T1''*hatB2+Xpsie*hatB2;',...
    'hatC12 hatD12;',...
    'hatC3*T2 hatD3;',...
    'zeros(',nx2,',',nx1,') eye(',nx2,') zeros(',nx2,',',nx3,')];'];
eval(str{sc});
...
str{sc}='Gamma_left21=sqrtm(Theta11)*[hatC3*T2 hatD3];';
eval(str{sc});
sc=sc+1;
...
if ~isinf(gain)
    ...
    str{sc}='Xi21=sqrtm(Theta11)*[hatC3*T2 hatC3 hatD3];';
    eval(str{sc});
    sc=sc+1;
    ...
    str{sc}=['mathB=[zeros(',ny1,',',nx1,') eye(',nx2,...
        ...
        'hatB'' zeros(',ny2,',',nx2,') hatE2''*Theta12 zeros(',...
        ny2,',',nx4,') hatE2''*sqrtm(Theta11)];'];
    eval(str{sc});
    sc=sc+1;
...
if ~vrb
    save iqc_estimation_l2gain_CTeli_yalmip_exe A A1 A2 A3 B1 B2 B3...
        Dqp Dqw Bp Bw C1 C2 D1 D2 Cq Cv Cy Dvp Dvw Dyp Dyw...
        lmiparameter A_W B_W C_W D_W
    if exist('state1','var')
        save iqc_estimation_l2gain_CTeli_yalmip_exe state* -append
    end
    if exist('struct1','var')
        save iqc_estimation_l2gain_CTeli_yalmip_exe struct* -append
    end
    if exist('l_1_1','var')
        save iqc_estimation_l2gain_CTeli_yalmip_exe l_* -append
    end
    if exist('r_1_1','var')
        save iqc_estimation_l2gain_CTeli_yalmip_exe r_* -append
    end
    if exist('MpiL_1_1','var')
        save iqc_estimation_l2gain_CTeli_yalmip_exe MpiL_* -append
    end
    if exist('MpiR_1_1','var')
        save iqc_estimation_l2gain_CTeli_yalmip_exe MpiR_* -append
    end
    fid=fopen('iqc_estimation_l2gain_CTeli_yalmip_exe.m','wt');
    for i=1:sc-1
        fprintf(fid,[str{i} '\n']);
    end
    fclose(fid);
