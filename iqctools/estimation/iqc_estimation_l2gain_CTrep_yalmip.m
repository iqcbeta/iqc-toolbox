function [gain,estimation]=...
    iqc_estimation_l2gain_CTrep_yalmip(w,v,y,p,q,weight)
% function [gain,estimation]=...
%     iqc_estimation_l2gain_CTrep_yalmip(w,v,y,p,q,weight)
%
%          ---------
...
switch ABST.systemtype
    case 'continuous',
        psi='[0 1;1 0]';
    case 'discrete',
        disp_str(70,'iqc_estimation_l2gain_CTrep_yalmip','discrete')
end
num_var_pos={};
...
if ~vrb
    disp_str(50,'iqc_estimation_l2gain_CTrep_yalmip')
else
    disp_str(51,'iqc_estimation_l2gain_CTrep_yalmip')
end
...
export_date=date;
str{1}=['%% Created by iqc_estimation_l2gain_CTrep_yalmip on ' export_date];
str{2}='\n%% Load coefficient matrices ...';
str{3}='load iqc_estimation_l2gain_CTrep_yalmip_exe';
...
nx1=num2str(size(Acle,2));
nx2=num2str(size(Bcle,2));

ny1=nx1;
ny2=num2str(size(Acle,1));
ny3=num2str(size(Mpi,1));
ny4=num2str(size(Cv,1));
ny5=num2str(size(hatB1,2));

str{sc}=['ouf_main1=[eye(',nx1,') zeros(',ny1,',',nx2,');',...
    'Acle+Aclpsi Bcle+Bclpsi;',...
    'Ccl Dcl];'];
eval(str{sc});
sc=sc+1;

str{sc}=['M_main1=[zeros(',ny1,',',ny1,') eye(',ny2,') zeros(',ny1,...
    ',',cal_str(ny3,ny4,ny5),');',...
    'eye(',ny2,') zeros(',ny2,',',cal_str(ny2,ny3,ny4,ny5),');',...
    'zeros(',ny3,',',cal_str(ny1,ny2),') Mpi zeros(',ny3,...
    ',',cal_str(ny4,ny5),');',...
    'zeros(',ny4,',',cal_str(ny1,ny2,ny3,ny4),') Theta12;',...
    'zeros(',ny5,',',cal_str(ny1,ny2,ny3),...
    ') Theta12'' -min_gamma*eye(',ny5,')];'];
eval(str{sc});
sc=sc+1;
...
str{sc}='Mcl21=sqrtm(Theta11)*[Ccl3 Dcl3];';
eval(str{sc});
sc=sc+1;
...
if ~vrb
    save iqc_estimation_l2gain_CTrep_yalmip_exe A A1 A2 A3 B1 B2 B3...
        Dqp Dqw Bp Bw C1 C2 D1 D2 Cq Cv Cy Dvp Dvw Dyp Dyw...
        lmiparameter A_W B_W C_W D_W
    if exist('state1','var')
        save iqc_estimation_l2gain_CTrep_yalmip_exe state* -append
    end
    if exist('struct1','var')
        save iqc_estimation_l2gain_CTrep_yalmip_exe struct* -append
    end
    if exist('l_1_1','var')
        save iqc_estimation_l2gain_CTrep_yalmip_exe l_* -append
    end
    if exist('r_1_1','var')
        save iqc_estimation_l2gain_CTrep_yalmip_exe r_* -append
    end
    if exist('MpiL_1_1','var')
        save iqc_estimation_l2gain_CTrep_yalmip_exe MpiL_* -append
    end
    if exist('MpiR_1_1','var')
        save iqc_estimation_l2gain_CTrep_yalmip_exe MpiR_* -append
    end
    fid=fopen('iqc_estimation_l2gain_CTrep_yalmip_exe.m','wt');
    for i=1:sc-1
        fprintf(fid,[str{i} '\n']);
    end
    fclose(fid);
