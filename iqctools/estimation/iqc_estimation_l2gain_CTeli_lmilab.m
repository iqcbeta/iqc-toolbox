function [gain,estimation]=...
    iqc_estimation_l2gain_CTeli_lmilab(w,v,y,p,q,weight)
% function [gain,estimation]=...
%     iqc_estimation_l2gain_CTeli_lmilab(w,v,y,p,q,weight)
%
%          ---------
...
switch ABST.systemtype
    case 'discrete',
        disp_str(70,'iqc_estimation_l2gain_CTeli_lmilab','discrete')
end
...
if vrb,
    if lmiparameter(5)>100,
        disp_str(50,'iqc_estimation_l2gain_CTeli_lmilab')
    else
        disp_str(51,'iqc_estimation_l2gain_CTeli_lmilab')
    end
end
...
export_date=date;
str{1}=['%% Created by iqc_estimation_l2gain_CTeli_lmilab on ' export_date];
str{2}='\n%% Load coefficient matrices ...';
str{3}='load iqc_estimation_l2gain_CTeli_lmilab_exe';
...
str{sc}=['lmiterm([',ks,' 1 2 X],1,1);'];
eval(str{sc});
sc=sc+1;

if size(A1,1)~=0
    str{sc}=['lmiterm([',ks,' 1 2 Xpsi],',ouf_str3,''',',ouf_str3,');'];
    eval(str{sc});
    sc=sc+1;
end
...
if size(hatA11,1)~=0
    str{sc}=['lmiterm([',ks,' 1 2 T11],',ouf_str1,...
        ''',',ouf_str1,'*hatC3''*Theta12*',ouf_str4,');'];
    eval(str{sc});
    sc=sc+1;
    str{sc}=['lmiterm([',ks,' 1 2 -T12],-',ouf_str2,...
        ''',',ouf_str1,'*hatC3''*Theta12*',ouf_str4,');'];
    eval(str{sc});
    sc=sc+1;
end
str{sc}=['lmiterm([',ks,' 1 2 0],',ouf_str2,...
...
%%
% [I;0]*Theta12'*D3
str{sc}=['lmiterm([',ks,' 2 2 0],(',ouf_str4,...
...
% [I;0]*Theta22*[I 0]
str{sc}=['lmiterm([',ks,' 2 2 min_gamma],-',ouf_str4,''',',ouf_str4,');'];
eval(str{sc});
sc=sc+1;
...
if size(hatA11,1)~=0
    str{sc}=['lmiterm([',ks,' 3 1 T11],sqrtm(Theta11)*hatC3*',ouf_str1,...
        ''',',ouf_str1,');'];
    eval(str{sc});
    sc=sc+1;
    str{sc}=['lmiterm([',ks,' 3 1 T12],-sqrtm(Theta11)*hatC3*',ouf_str1,...
        ''',',ouf_str2,');'];
    eval(str{sc});
    sc=sc+1;
end
str{sc}=['lmiterm([',ks,' 3 1 0],sqrtm(Theta11)*hatC3*',ouf_str2,...
    '''*',ouf_str2,');'];
eval(str{sc});
sc=sc+1;

%% Theta11^(1/2)*D3
str{sc}=['lmiterm([',ks,' 3 2 0],sqrtm(Theta11)*hatD3);'];
eval(str{sc});
sc=sc+1;

%% -I
str{sc}=['lmiterm([',ks,' 3 3 0],-eye(',nc3,'));'];
eval(str{sc});
sc=sc+1;
...
%%
str{sc}='NWl=[hatB'' hatE2''*Theta12  hatE2''*sqrtm(Theta11)];';
eval(str{sc});
sc=sc+1;
...
nx2=num2str(size(hatB2,2));
ny1=num2str(size(Wl1,1));
ny2=num2str(size(Wl2,1));
ny3=nx2;
ny4=num2str(size(Wl3,1));

str{sc}=['ouf_main_left=[Wl1 zeros(',ny1,',',nx2,');',...
    'Wl2 zeros(',ny2,',',nx2,');',...
    'zeros(',ny3,',',nx1,') eye(',nx2,');',...
    'Wl3 zeros(',ny4,',',nx2,')];'];
eval(str{sc});
sc=sc+1;
...
if ~isinf(gain)
    ...
    str{sc}=['ouf_Xi11=[eye(',cal_str(nx1,nx2),') zeros(',cal_str(nx1,nx2),...
        ',',cal_str(nx3,nx4),');',...
        'T1''*hatA*T2+Xpsie*hatA T1''*hatA+Xpsie*hatA T1''*hatB1+Xpsie*hatB1 ',...
        'T1''*hatB2+Xpsie*hatB2;',...
        'Xpsie*hatA X*hatA+Xpsie*hatA X*hatB1+Xpsie*hatB1 X*hatB2+Xpsie*hatB2;',...
        'hatC12 hatC12 hatD12;',...
        'hatC3*T2 hatC3 hatD3;',...
        'zeros(',nx3,',',cal_str(nx1,nx2),') eye(',nx3,') zeros(',nx3,...
        ',',nx4,')];'];
    eval(str{sc});
    sc=sc+1;
    
    str{sc}='Xi21=[hatC3*T2 hatC3 hatD3];';
    eval(str{sc});
    sc=sc+1;
    ...
    str{sc}=['mathC=[eye(',ny1,') zeros(',ny1,',',cal_str(nx2,nx3,nx4,nx5),');',...
    ...
    nx1=num2str(size(hatB',2));
    nx2=num2str(size(hatA,2));
    nx3=num2str(size(hatB1,2));
    nx4=num2str(size(hatB2,2));
    nx5=ny2;
    str{sc}=['mathB=[zeros(',ny1,',',nx1,') eye(',nx2,...
        ') zeros(',ny1,',',cal_str(nx3,nx4,nx5),');',...
        'hatB'' zeros(',ny2,',',nx2,') hatE2''*Theta12 zeros(',ny2,',',...
        nx4,') hatE2''*sqrtm(Theta11)];'];
    eval(str{sc});
    sc=sc+1;
        ...
        for i=1:size(E.X{n},2),
            ...
            if vr>0
                str{sc}=['Mpi=Mpi+MpiL_',js,'_',is,'*decs2mats(xopt,sX',...
                    num2str(vr),')*MpiR_',js,'_',is,'+(MpiL_',js,'_',is,'*decs2mats(xopt,sX',...
    ...
    nx1=num2str(2*size(hatA,1));
    nx2=nx1;
    nx3=num2str(size(Pr0,1));
    str{sc}=['M_Xi11=[zeros(',nx1,',',nx1,') eye(',nx1,') zeros(',nx1,',',nx3,');',...
        'eye(',nx2,') zeros(',nx2,',',cal_str(nx2,nx3),');',...
        'zeros(',nx3,',',cal_str(nx1,nx2),') Pr0];'];
    eval(str{sc});
    sc=sc+1;
...
if lmiparameter(5)>100;
    save iqc_estimation_l2gain_CTeli_lmilab_exe A A1 A2 A3 B1 B2 B3...
        Dqp Dqw Bp Bw C1 C2 D1 D2 Cq Cv Cy Dvp Dvw Dyp Dyw...
        lmiparameter A_W B_W C_W D_W
    if exist('state1','var')
        save iqc_estimation_l2gain_CTeli_lmilab_exe state* -append
    end
    if exist('struct1','var')
        save iqc_estimation_l2gain_CTeli_lmilab_exe struct* -append
    end
    if exist('l_1_1','var')
        save iqc_estimation_l2gain_CTeli_lmilab_exe l_* -append
    end
    if exist('r_1_1','var')
        save iqc_estimation_l2gain_CTeli_lmilab_exe r_* -append
    end
    if exist('MpiL_1_1','var')
        save iqc_estimation_l2gain_CTeli_lmilab_exe MpiL_* -append
    end
    if exist('MpiR_1_1','var')
        save iqc_estimation_l2gain_CTeli_lmilab_exe MpiR_* -append
    end
    fid=fopen('iqc_estimation_l2gain_CTeli_lmilab_exe.m','wt');
    for i=1:sc-1
        fprintf(fid,[str{i} '\n']);
    end
    fclose(fid);
