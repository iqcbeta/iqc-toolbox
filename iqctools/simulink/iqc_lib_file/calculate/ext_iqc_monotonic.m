function ext_iqc_monotonic(block)
setup(block);
...
str=['plot(0,0,100,100,[99,-0],[50,50],[50,50],[-0,100],[10,90],',...
    '[0,100],[99,95,88,82,75,70,67,60,55,50],[76,76,75,74,72,',...
    '70,68,62,56,50],[1,5,9,16,22,29,34,40,45,50],',...
    '[20,21,22,24,26,29,32,38,44,50]);'];
set_param(gcb,'MaskDisplay',str);

function CheckParameters(block)
pole = block.DialogPrm(1).Data;
int_dd = block.DialogPrm(2).Data;
ope = block.DialogPrm(3).Data;
out_var = block.DialogPrm(4).Data;
if ~isa(pole,'double')
    DAStudio.error('Simulink:block:invalidParameter');
end
if ~isa(int_dd,'double') || length(int_dd)~=1
    DAStudio.error('Simulink:block:invalidParameter');
end
if ~isa(ope,'double')
    DAStudio.error('Simulink:block:invalidParameter');
...
glparam_name='ext_iqc_monotonic';
pos=checkblockpos(block_path,glparam_name);
alparam_name={'name','pole','int_dd','ope','outvariable','path','newpath'};
alparam={block_name,pole,int_dd,ope,str,block_path,block_newpath};
...
ext_iqc_setvalues.ext_iqc_monotonic.sampletime{pos}(1:2)=di(1);
...
ext_iqc_setvalues.ext_iqc_monotonic.size{pos}(1:2)=di;
...
if ~isfield(ext_iqc_setvalues.ext_iqc_monotonic,'size')
    ext_iqc_setvalues.ext_iqc_monotonic.size{pos}(1:2)=...
